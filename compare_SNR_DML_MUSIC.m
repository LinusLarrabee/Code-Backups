...
d=0.5*lamdba;%阵元间距
Gaussion_noise=randn(sensor_number,N_x)+1j*randn(sensor_number,N_x);error_t = zeros(1,21);error=0;
for kk=1:21
    %snr=-20;%信噪�?
    snr=2*(kk-11);
    source_number=3;%信元�?
    ...
    %     figure;plot(searching_doa,10*log10(abs(PSML./max(PSML))),searching_doa(maxl),maxv,'*');
        ...
        sita_error_average(kk)=sum(he)/length(source_doa);
        error = error + sita_error_average(kk);
        sita_error_average2(kk)=sum(he2)/length(source_doa);
    end
    error_t(1,kk)    =   error/5;
    error = 0;
end
figure;
plot(-20:2:20,error_t,'r*-');hold on;plot(-20:2:20,sita_error_average2,'b*-');
title('����SNR������Ƕ����Ĺ�ϵ');xlabel('SNR/dB');ylabel('�Ƕ����/degree');
